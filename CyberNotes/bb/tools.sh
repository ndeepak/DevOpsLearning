...
# Install Massdns
# Redirect build process output:
print_status "Installing MassDNS..."
if [ ! -d "$HOME/tools/massdns" ]; then
  git clone https://github.com/blechschmidt/massdns.git "$HOME/tools/massdns" >/dev/null 2>&1
  cd "$HOME/tools/massdns" || exit
  make > /dev/null 2>&1 && sudo make install > /dev/null 2>&1
else
  echo -e "${YELLOW}[!] Massdns is already installed. Skipping...${RESET}"
fi
