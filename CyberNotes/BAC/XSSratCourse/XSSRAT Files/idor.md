...
2. **Parameter Manipulation Techniques**:
   ...
   - **Parameter Name Replacement**: Try alternative parameter names (e.g., album_id → account_id)
   ...
   - **New Feature Focus**: Pay special attention to newly added features as they may have weaker access controls
...
- GraphQL often bundles multiple objects in one request; enforce authorization per-field, not just at the root resolver.
- Disable introspection in production and fuzz `__typename` switches to reach sibling object types.
- Swap out `id` arguments and nested object IDs in mutations and batch queries.

### gRPC / Protobuf

- Micro-services have shifted from REST to gRPC; object IDs sit in binary messages.
- Use Burp Suite 2024.2’s _gRPC Inspector_ or `grpc-fuzzer` to enumerate and tamper with message fields.
- If server reflection is enabled, fetch `.proto` definitions automatically.

### Cloud-Native Object URLs

- Pre-signed URLs for S3, GCS or Azure Blob are direct object references; short expiries and unguessable keys are critical.
- Attempt to tweak the `Key=` parameter or manipulate the `X-Amz-Security-Token` to replay someone else’s file.
...
- Many organisations delegate authorization to policy engines such as **Open Policy Agent (OPA)** or **AWS Cedar**.
- Send crafted inputs directly to policy endpoints (e.g., `POST /v1/data/authz/allow`) to detect overly-permissive rules.
- Watch for missing “object owner” checks in Rego or Cedar policies that enable vertical privilege escalation.

## Modern Detection & CI/CD Tooling

- **Burp Suite 2025** – adds gRPC Inspector and Parallel Repeater that auto-tracks session tokens.
- **Autorize 2.0 (2025)** – revamped Burp extension that learns dynamic cookies and headers for diff-based auth testing.
- **Pynt (2024)** – open-source CLI/CI scanner that generates BOLA test cases for every endpoint.
- **StackHawk (2024)** – CI-native scanner; its BOLA module blocks merges on failing IDOR checks.
