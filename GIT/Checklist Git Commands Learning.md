...
- [x]  Understand the concept of version control and its importance in software development.
...
- Linux/macOS: Use `sudo apt install git` (Debian) or `brew install git` (Mac)

- [x]  Configure Git with your name and email:
    `git config --global user.name "Your Name" `
    `git config --global user.email "your.email@example.com"
	`git config --global --list

- [x]  Understand the basic Git workflow: working directory, staging area, and repository.
- **Working Directory** → Local files
- **Staging Area** → Files prepared for commit
- **Repository** → Saved versions of files

#### **3. Repository Management**
- [x]  Initialize a new Git repository:    
    `git init`
- [x]  Clone an existing repository:
    `git clone <repository-url>`
    
- [x] Clone a specific branch:
`git clone -b <branch-name> <repository-url>`

#### **4. Tracking and Staging Changes**
- [x]  Check the status of your working directory:
    `git status`
- [x]  Track new files and changes:
    `git add <file> 
    `git add .`
- [x]  View changes in the working directory:
    `git diff`
...
- [ ]  Amend the most recent commit:
    `git commit --amend`
- [ ] Commit all tracked files (skipping `git add`):
...
- [ ]  Use `hotfix/` branches for urgent fixes in production.
- [ ]  Merge feature branches into `develop` before merging into `main`.